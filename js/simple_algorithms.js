(function(){
  ...
  function floyd_warshall(g, source) {
    ...
    function getPath(i, j) {
      ...
      var left  = getPath(i, intermediate);
      var right = getPath(intermediate, j);
      return left.concat([intermediate]).concat(right);
    }
    ...
    for (i in g.nodes) {
      road[i] = []
      for (j in g.nodes) {
        road[i][j] = getPath(i,j);
      }
    }

    return { path: path, next: next, road: road }
  ...
  */
  ...
  if (typeof(module) !== "undefined") {
    ...
    this["bellman_ford"]          = bellman_ford;
